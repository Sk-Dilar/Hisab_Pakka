...
const EditWorkItemModal = ({ open, onClose, workItem }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateWorkItem({ id: workItem._id, title: formData.title, quantity, rate }).unwrap();
      onClose();
    } catch (err) {
      setErrorMsg(err.data?.message || 'Failed to update work item');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm animate-slide-up overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h3 className="font-bold text-[#1a1f36]">Edit Work Item</h3>
            <p className="text-xs text-slate-400">Update unbilled task details</p>
          </div>
