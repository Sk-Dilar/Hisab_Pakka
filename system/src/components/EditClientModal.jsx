...
import { useUpdateClientMutation } from '../store/api/apiSlice';
...
const EditClientModal = ({ open, onClose, client }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMsg(err.data?.message || 'Failed to update client');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-slide-up overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h3 className="font-bold text-[#1a1f36]">Edit Client</h3>
            <p className="text-xs text-slate-400">Update this client's details</p>
          </div>
