...
import { FiX, FiAlertCircle, FiBriefcase, FiFileText } from 'react-icons/fi';
import { useUpdateProjectMutation } from '../store/api/apiSlice';
...
const EditProjectModal = ({ open, onClose, project }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.title.trim()) { setErrorMsg('Project title is required'); return; }
    try {
      await updateProject({ id: project._id, ...formData }).unwrap();
      onClose();
    } catch (err) {
      setErrorMsg(err.data?.message || 'Failed to update project');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-slide-up overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h3 className="font-bold text-[#1a1f36]">Edit Project</h3>
